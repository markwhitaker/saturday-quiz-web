...
public class GuardianApiClientTests
{
    ...
    public async Task GivenValidUrl_WhenGetQuizMetadataAsync_ThenExpectedMetadataIsReturned()
    {
        ...
        // When
        var actualMetadata = await _guardianApiClient.GetQuizMetadataAsync(expectedPageCount);

        // Then
        Assert.That(actualMetadata.Count, Is.EqualTo(expectedPageCount));
    ...
    [Test]
    public async Task GivenInvalidUrl_WhenGetQuizMetadataAsync_ThenEmptyMetadataIsReturned()
    {
        // Given
        var expectedConfig = new GuardianConfig
        {
